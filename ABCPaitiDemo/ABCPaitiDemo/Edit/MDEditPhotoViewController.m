...
- (void)setImage:(UIImage *)image
{
    ...
    NSLog(@"mdEditPhotoVC set image:%@", NSStringFromCGSize(image.size));
}
...
{
//    NSLog(@"touchesBegan: %@ event: %@", touches, event);
...
{
//    NSLog(@"touchesMoved: %@ event: %@", touches, event);
...
{
//    NSLog(@"touchesEnded: %@ event: %@", touches, event);
...
{
//    NSLog(@"touchesCancelled: %@ event: %@", touches, event);
...
{
    BOOL handle = YES;
//    NSLog(@"handleGestureState: %li", state);
    ...
    switch (state) {
        ...
        case UIGestureRecognizerStateEnded: {
            self.gestureCount--;
            handle = NO;
            NSLog(@"gestureCount: %lu", (unsigned long)self.gestureCount);
...
//                    [self adjustCaptureImageViewFrame];
                    ...
                    self.view.userInteractionEnabled = NO;
                    NSLog(@"handleGestureState end before assign:%@", NSStringFromCGRect(self.captureImageView.frame));
                    
                    [UIView animateWithDuration:kAnimationIntervalTransform delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
                        NSLog(@"handleGestureState end capTran:%@ validTran:%@", NSStringFromCGAffineTransform(self.captureImageView.transform), NSStringFromCGAffineTransform(self.validTransform));
                        
                        self.captureImageView.transform = self.validTransform;
                        
                        NSLog(@"handleGestureState end after assign block:%@", NSStringFromCGRect(self.captureImageView.frame));
                    } completion:^(BOOL finished) {
                        self.view.userInteractionEnabled = YES;
                    }];
...
//    }
    ...
    if( CGRectContainsRect( rectR3, r1 ) ) {
        NSLog(@"first rect contains second rect");
        self.validTransform = transform;
    }
}
...
//        NSLog(@"location in ClipPanel: %@", NSStringFromCGRect(self.clipPanel.clippingRect));
        ...
        if(handle) {
//            NSLog(@"handlePan before: %@", NSStringFromCGRect(self.captureImageView.frame));
...
- (void) handlePinch:(UIPinchGestureRecognizer *)recognizer
{
    if([self handleGestureState:recognizer.state]) {
        ...
        NSLog(@"handlePinch before: %@", NSStringFromCGRect(self.captureImageView.frame));
        ...
        NSLog(@"handlePinch after: %@", NSStringFromCGRect(self.captureImageView.frame));
...
- (void)adjustCaptureImageViewFrame
{
    ...
    NSLog(@"adjustCaptureImageViewFrame imgV transform:%@", NSStringFromCGAffineTransform(self.captureImageView.transform));
    
    CGRect imgVFrame = self.captureImageView.frame;
    NSLog(@"imgVFrame:%@", NSStringFromCGRect(imgVFrame));
    ...
    NSLog(@"RECT_CROP: %@ imageSize: %@ adjFrame:%@",
    ...
    [UIView animateWithDuration:0.3 animations:^{
        CGRect newFrame = CGRectMake(CGRectGetMidX(RECT_CROPFRAME) - (newWidth / 2),
                                     ...
                                     newWidth,
                                     newHeight);
        NSLog(@"newFrame: %@ scale: %f xDis: %f yDis: %f", NSStringFromCGRect(newFrame), scale, xDistance, yDistance);
...
//        NSLog(@"scrFrame:%@ conSize:%@", NSStringFromCGRect(_scrollView.frame), NSStringFromCGSize(_scrollView.contentSize));
//        
...
//                         self.scrollView.layer.transform = [self rotateTransform:CATransform3DIdentity];
                         NSLog(@"captureIV before rotate left: %@", NSStringFromCGRect(self.captureImageView.frame));
                         ...
                         NSLog(@"captureIV after rotate left: %@", NSStringFromCGRect(self.captureImageView.frame));
...
//    [XueXiBao trackEvent:EVENT_SUB_EDIT_OK];
    ...
    if (!self.captureImageView.image) {
        NSLog(@"EditPhoto capture image nil");
        return;
    }
    ...
    NSLog(@"ImgV Frame:%@ imageSize:%@",
          NSStringFromCGRect(self.captureImageView.frame),
          NSStringFromCGSize(self.captureImageView.image.size));
    NSLog(@"superViewTranslate:%@", NSStringFromCGPoint(superViewTranslate));

    
    NSLog(@"relativeClipRect:%@", NSStringFromCGRect(rct));
    rct.size.width /= scale;
    rct.size.height /= scale;
    rct.origin.x /= scale;
    rct.origin.y /= scale;
    NSLog(@"finalClipRect:%@", NSStringFromCGRect(rct));
    
    
    CGRect finalCropRct = [self computeRealCropRect:rct withinSize:self.captureImageView.image.size];
    NSLog(@"real Croprect: %@", NSStringFromCGRect(finalCropRct));
    
    UIImage *image = [ABCImageUtil crop:_oriImage rect:finalCropRct]; //[self.captureImageView.image crop:rct];
